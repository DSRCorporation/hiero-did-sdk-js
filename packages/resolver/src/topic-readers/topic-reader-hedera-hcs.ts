...
import {
  HederaHcsService,
  HederaHcsServiceConfiguration
} from '@hiero-did-sdk/hcs';
...
export class TopicReaderHederaHcs extends TopicReader {
  ...
  constructor(config: HederaHcsServiceConfiguration) {
    ...
    if (!config.networks.every(net => typeof net.network === 'string'
      && ['mainnet', 'testnet', 'previewnet', 'local-node'].includes(net.network))
    ) {
  ...
  async fetchFrom(
    ...
    const messages = await this.hcsService.getTopicMessages({topicId, networkName: network, toDate: new Date(options.to)})
